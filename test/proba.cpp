...
void myTimerEvent()
{
  if ((digitalRead(button1) == HIGH) && (ledState1 == LOW)){
   ...
   if ((digitalRead(button1) == LOW) && (ledState1 == HIGH)){
     ledState1 = LOW;
     pcf8574.digitalWrite(4, ledState1);
     //Blynk.virtualWrite(V2, ledState1);
     Serial.println("led1 off");
   }
   if ((digitalRead(button2) == LOW) && (ledState2 == HIGH)){
     ledState2 = LOW;
     pcf8574.digitalWrite(5, ledState2);
    // Blynk.virtualWrite(V3, ledState2);
     Serial.println("led2 off");
   }
   if ((digitalRead(button3) == LOW) && (ledState3 == HIGH)){
     ledState3 = LOW;
     pcf8574.digitalWrite(6, ledState3);
     //Blynk.virtualWrite(V4, ledState3);
     Serial.println("led3 off");
   }
   if ((pcf8574.digitalRead(P0) == LOW) && (GirlandState == HIGH)){
     GirlandState = LOW;
     pcf8574.digitalWrite(7, GirlandState);
     //Blynk.virtualWrite(V5, GirlandState);
